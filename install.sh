#!/usr/bin/env bash
# OpsClaw Tools Install Script (Linux + macOS)
# Usage: curl -fsSL https://raw.githubusercontent.com/lbjlaq/OpsClaw-Manager/main/install.sh | bash
#
...
APP_NAME="OpsClaw Tools"
APP_ID="com.lbjlaq.opsclaw-tools"
...
get_version() {
    ...
    local response
    if response=$(curl -fsSL -H "User-Agent: OpsClaw-Installer" "${GITHUB_API}/latest" 2>/dev/null); then
...
build_download_url() {
    ...
    case "$PLATFORM" in
        linux)
            case "$PKG_EXT" in
                deb)
                    # OpsClaw.Tools_4.1.20_amd64.deb or _arm64.deb
                    DOWNLOAD_URL="${base_url}/OpsClaw.Tools_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    FILENAME="OpsClaw.Tools_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    ;;
                rpm)
                    # OpsClaw.Tools-4.1.20-1.x86_64.rpm or -1.aarch64.rpm
                    DOWNLOAD_URL="${base_url}/OpsClaw.Tools-${RELEASE_VERSION}-1.${RPM_ARCH}.rpm"
                    FILENAME="OpsClaw.Tools-${RELEASE_VERSION}-1.${RPM_ARCH}.rpm"
                    ;;
                AppImage)
                    # OpsClaw.Tools_4.1.20_amd64.AppImage or _aarch64.AppImage
                    local appimage_arch
                    ...
                    DOWNLOAD_URL="${base_url}/OpsClaw.Tools_${RELEASE_VERSION}_${appimage_arch}.AppImage"
                    FILENAME="OpsClaw.Tools_${RELEASE_VERSION}_${appimage_arch}.AppImage"
                    ;;
            esac
            ;;
        macos)
            # Prefer universal DMG, fallback to arch-specific
            DOWNLOAD_URL="${base_url}/OpsClaw.Tools_${RELEASE_VERSION}_universal.dmg"
            FILENAME="OpsClaw.Tools_${RELEASE_VERSION}_universal.dmg"
            ;;
    esac
...
install_linux() {
    ...
    case "$PKG_MANAGER" in
        ...
        appimage)
            ...
            run cp "$DOWNLOAD_PATH" "${install_dir}/opsclaw-tools"

            if [[ ":$PATH:" != *":${install_dir}:"* ]]; then
                warn "Add ${install_dir} to your PATH to run opsclaw-tools from anywhere"
