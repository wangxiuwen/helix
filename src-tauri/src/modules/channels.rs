...
pub async fn route_inbound_message(msg: &InboundMessage) -> Result<String, String> {
    ...
    let reply = super::agent::agent_process_message(&msg.session_key, &msg.content).await?;
...
pub async fn dispatch_outbound_message(msg: &OutboundMessage) -> Result<(), String> {
    match &msg.channel {
        ChannelId::WeChatFileHelper => {
            super::filehelper::filehelper_send_msg(msg.session_key.clone(), msg.content.clone())
                .await
                .map(|_| ())
        }
        ChannelId::Feishu => {
            let config = super::load_app_config().map_err(|e| e.to_string())?;
            if let Some(ref notif) = config.notifications {
                if let Some(ref url) = notif.feishu_webhook {
                    super::notifications::send_feishu(url, "Helix", &msg.content).await
                } else {
        ...
        ChannelId::DingTalk => {
            ...
            if let Some(ref notif) = config.notifications {
                if let Some(ref url) = notif.dingtalk_webhook {
                    super::notifications::send_dingtalk(url, "Helix", &msg.content).await
                } else {
