...
async fn check_updater_json() -> Result<UpdateInfo, String> {
    ...
    let download_url = format!("https://github.com/lbjlaq/Helix-Manager/releases/tag/v{}", latest_version);
...
async fn check_static_url(url: &str, source_name: &str) -> Result<UpdateInfo, String> {
    ...
    let download_url = "https://github.com/lbjlaq/Helix-Manager/releases/latest".to_string();
...
pub fn is_homebrew_installed() -> bool {
    ...
    {
        let caskroom_paths = [
            "/opt/homebrew/Caskroom/helix-tools",
            "/usr/local/Caskroom/helix-tools",
        ];
...
/// Execute `brew upgrade --cask helix-tools` with timeout (macOS only)
...
#[cfg(target_os = "macos")]
pub async fn brew_upgrade_cask() -> Result<String, String> {
    logger::log_info("Starting Homebrew Cask upgrade for helix-tools...");
