...
```bash
# 查看日志
tail -f ~/Library/Application\ Support/com.opsclaw.tools/logs/opsclaw.log | grep "Layer-1"
```
...
```bash
# 查看签名保留情况
tail -f ~/Library/Application\ Support/com.opsclaw.tools/logs/opsclaw.log | grep -E "(Layer-2|signature)"
```
...
```bash
# 查看 Layer 3 触发和摘要生成
tail -f ~/Library/Application\ Support/com.opsclaw.tools/logs/opsclaw.log | grep -E "(Layer-3|XML summary|Fork)"
```
...
```bash
# 查看所有层级的触发
tail -f ~/Library/Application\ Support/com.opsclaw.tools/logs/opsclaw.log | grep -E "Layer-[123]"
```
...
```bash
# 查看错误处理
tail -f ~/Library/Application\ Support/com.opsclaw.tools/logs/opsclaw.log | grep -E "(Layer-3.*failed|BAD_REQUEST)"
```
...
```bash
# Layer 1 触发
grep "Layer-1.*Tool trimming" opsclaw.log

# Layer 2 触发
grep "Layer-2.*Thinking compression" opsclaw.log

# Layer 3 触发
grep "Layer-3.*Fork successful" opsclaw.log

# Token 节省统计
grep "Compression result.*saved" opsclaw.log
```
...
echo "=== 查看日志 ==="
tail -100 ~/Library/Application\ Support/com.opsclaw.tools/logs/opsclaw.log | grep -E "Layer-[123]"
```
...
https://github.com/lbjlaq/OpsClaw-Manager/issues
