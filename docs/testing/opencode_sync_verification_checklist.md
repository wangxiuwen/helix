...
- [ ] Verify backup suffix: `.opsclaw-manager.bak` (new) and `.opsclaw.bak` (legacy)
- [ ] Verify backup location: `~/.config/opencode/opencode.json.opsclaw-manager.bak`
- [ ] Verify accounts backup: `~/.config/opencode/opsclaw-accounts.json.opsclaw-manager.bak`
...
- [ ] `provider.opsclaw-manager` created with correct structure
- [ ] `npm`: `@ai-sdk/anthropic`
- [ ] `name`: `OpsClaw Manager`
...
```
- [ ] File created at `~/.config/opencode/opsclaw-accounts.json`
- [ ] `version` field = 3
...
```bash
opencode run "test" --model opsclaw-manager/claude-sonnet-4-5-thinking --variant high
```
...
```bash
opencode run "test" --model opsclaw-manager/gemini-3-pro-high --variant low
```
...
```bash
# If opencode-opsclaw-auth plugin installed
opencode run "test" --model google/opsclaw-claude-sonnet-4-5-thinking --variant max
```
...
### 5.1 Clear Config
- [ ] Removes `provider.opsclaw-manager`
...
|-------------|-----------------|
| New suffix (`.opsclaw-manager.bak`) | Restores successfully |
| Old suffix (`.opsclaw.bak`) | Restores successfully (backward compatible) |
