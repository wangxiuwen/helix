# 🐋 OpsClaw Manager 原生 Docker 部署手冊

本目錄包含 OpsClaw Manager 的原生 Headless Docker 部署方案。該方案支持完整的 Web 管理界面、API 反代以及數據持久化，無需複雜的 VNC 或桌面環境。
...
> *   **查看方式**：執行 `docker logs opsclaw-manager` 尋找 `Current API Key` 或 `Web UI Password`，或執行 `grep -E '"api_key"|"admin_password"' ~/.opsclaw_tools/gui_config.json` 查看。
...
docker run -d \
  ...
  -v ~/.opsclaw_tools:/root/.opsclaw_tools \
  lbjlaq/opsclaw-manager:latest
```
...
```bash
# 默認構建最新標籤
docker build -t opsclaw-manager:latest -f docker/Dockerfile .
```
...
```bash
# 強制使用国内镜像加速構建
docker build --build-arg USE_MIRROR=true -t opsclaw-manager:latest -f docker/Dockerfile .
```
...
## 📂 數據持久化
請務必將宿主機目錄掛載至容器內的 `/root/.opsclaw_tools`，否則賬號和配置在容器重啟後會丟失。
...
```bash
# 打上版本標籤並推送
docker tag opsclaw-manager:latest lbjlaq/opsclaw-manager:latest
docker tag opsclaw-manager:latest lbjlaq/opsclaw-manager:4.1.20
docker push lbjlaq/opsclaw-manager:latest
docker push lbjlaq/opsclaw-manager:4.1.20
```
